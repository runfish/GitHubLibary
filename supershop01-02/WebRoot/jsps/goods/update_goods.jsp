...
#oldimage {
	...
	<body>
		<div id="wrapper">
			<div id="f_title">
				商品[${goods.goodsName}]信息修改
			</div>
			<img id="oldimage"
				src="<c:url value="/action/goodsMgr?act=getimage"/>&goodsid=${goods.goodsId}" />
			<form action="<c:url value="/action/goodsMgr"/>" method="post"
				enctype="multipart/form-data" onSubmit="return validateEntry(this);">
				...
				<div class="f_row">
					...
					<select name="goodsctgid">
						...
						<c:forEach var="category" items="${ctgList}">
							<option value="${category.smallCtgId}"
								<c:if test="${goods.goodsCtgId==category.smallCtgId}">selected</c:if>>
								${category.smallCtgName}
							</option>
						</c:forEach>
					</select>
				</div>
				<div class="f_row">
					<span>商品库存数量</span>
					<input type="text" name="goodsqty" value="${goods.goodsQty}"
						size="20" />
				</div>
				<div class="f_row">
					<span>商品所属管理员</span>
					<select name="goodsadminid">
						...
						<c:forEach var="administrator" items="${administrators}">
							<option value="${administrator.administratorId}"
								<c:if test="${goods.goodsAdminId==administrator.administratorId}">selected</c:if>>
